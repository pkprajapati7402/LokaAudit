...
# Build Docker images for LokaAudit test execution
...
# Build Solana test runner (alias to rust-test-runner with Solana tools)
echo "Building Solana test runner..."
docker build -t lokaudit/solana-test-runner:latest ./docker/rust-test-runner/

# Tag with additional names for compatibility
...
docker images | grep -E "(lokaudit|rust-test-runner|solana-test-runner)"

echo ""
echo "To run test execution manually:"
echo "docker run --rm -v \$(pwd):/workspace -w /workspace lokaudit/rust-test-runner:latest cargo test"

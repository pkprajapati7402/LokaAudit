...
export async function parseTypeScriptAST(sourceCode: string, fileName: string): Promise<ParsedAST> {
  try {
 ...
 */
function parseGenericAST(sourceCode: string, fileName: string): ParsedAST {
  const result: ParsedAST = {
    name: extractModuleName(fileName, 'rust'),
    module_type: 'rust_crate',
    functions: [],
